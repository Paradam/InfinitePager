 ...
 * behave as a normal {@link android.support.v13.app.FragmentPagerAdapter FragmentPagerAdapter} <p/>
 *
 * <p>This class is intended to be used in applications supporting API {@link android.os.Build.VERSION_CODES#HONEYCOMB_MR2 13}
 * and above using the native Android {@link android.app.Fragment Fragment} class. For applications
 * supporting APIs bellow {@link android.os.Build.VERSION_CODES#HONEYCOMB_MR2 13} or using
 ...
 * {@link android.support.v13.app.FragmentPagerAdapter} that represents each page as a {@link android.app.Fragment}
 ...
 * static fragments to be paged through, such as a set of tabs. The fragment of each page the user
 * visits will be kept in memory, though its view hierarchy may be destroyed when not visible. This
 * can result in using a significant amount of memory since fragment instances can hold on to an
 * arbitrary amount of state. For larger sets of pages, consider {@link com.github.paradam.infinitepager.InfiniteFragmentStatePagerAdapter}.<p/>
 *
...
public abstract class InfiniteFragmentPagerAdapter extends InfinitePagerAdapter {
    ...
    /**
     * Return a unique identifier for the item at the given position. <p/> <p>The default
     * implementation returns the given position. Subclasses should override this method if the
     * positions of items can change.</p>
     *
    ...
    /**
     * Return a unique identifier for the item at the given position. <p/> <p>{@link
     * #getRelativeItemId(int)} should be used instead to get the Appropriate item id from the item
     * in the implementing classes list.</p>
     *
     * @param position Position within this adapter
     * @return Unique identifier for the item at position
     * @see #getRelativeItemId(int)
     */
