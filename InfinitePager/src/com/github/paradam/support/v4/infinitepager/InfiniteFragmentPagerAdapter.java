 ...
 * {@link android.support.v4.app.FragmentPagerAdapter FragmentPagerAdapter}
 * 
 * <p>Applications targeting {@link Build.VERSION_CODES#DONUT} and above should use this class,
 * applications targeting only {@link Build.VERSION_CODES#HONEYCOMB_MR2 13} and above using the
 * native Android {@link android.app.Fragment Fragment} class should use instead 
 ...
 * {@link android.support.v4.app.FragmentPagerAdapter} that represents each page
 * as a {@link Fragment} that is persistently kept in the fragment manager as long as
 * the user can return to the page.
 *
 * <p>This version of the pager is best for use when there are a handful of typically more
 * static fragments to be paged through, such as a set of tabs. The fragment of each page
 * the user visits will be kept in memory, though its view hierarchy may be destroyed when
 * not visible.  This can result in using a significant amount of memory since fragment
 * instances can hold on to an arbitrary amount of state. For larger sets of pages, consider
 * {@link InfiniteFragmentStatePagerAdapter}.
 *
 ...
 * to have a working adapter. Override {@link #getRelativeItemId(int)} if the fragments have
 * the possibility of changing positions within the ViewPager.
 * 
 * @author Adam Parr
 *
 */
public abstract class InfiniteFragmentPagerAdapter extends InfinitePagerAdapter {
	 ...
	 * Return a unique identifier for the item at the given position.
	 * 
	 * <p>{@link #getRelativeItemId(int)} should be used instead to get the
	 * Appropriate item id from the item in the implementing classes list.</p>
	 *
	 * @param position Position within this adapter
	 * @return Unique identifier for the item at position
	 */
	public long getItemId(int position) {
		return getRelativeItemId(getRelitivePosition(position));
	}
